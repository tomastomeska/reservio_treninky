    ...
    if (!verifyCsrf($_POST['csrf_token'] ?? '')) {
        ...
        if ($firstName === '' || $lastName === '') {
            ...
            if ($newPhoto !== null) {
                ...
                $stmt = $pdo->prepare(
                    'UPDATE athletes SET first_name=?, last_name=?, birth_date=?, email=?, notes=?, photo=?
                     WHERE id=? AND coach_id=?'
                );
                $stmt->execute([
                    $firstName, $lastName, $birthDate ?: null, $email ?: null, $notes ?: null,
                    $newPhoto, $athleteId, $coachId,
                ]);
            } else {
                $stmt = $pdo->prepare(
                    'UPDATE athletes SET first_name=?, last_name=?, birth_date=?, email=?, notes=?
                     WHERE id=? AND coach_id=?'
                );
                $stmt->execute([
                    $firstName, $lastName, $birthDate ?: null, $email ?: null, $notes ?: null,
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4">
                <form method="post" enctype="multipart/form-data" novalidate>
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-sm-4">
                            <label class="form-label fw-semibold">Datum narození</label>
                            <input type="date" name="birth_date" class="form-control"
                                   value="<?= h($d['birth_date'] ?? '') ?>" max="<?= date('Y-m-d') ?>">
                        </div>
                        <div class="col-sm-8">
